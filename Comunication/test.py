...
var_unit = ["[0.00-360.00] Degree", "[0.00-360.00] Degree",
            "[0.00-250.00] mm", "[0.00-360.00] Degree"]
name_xyz = ["x", "y", "z", "roll"]
var_unit_xyz = ["[0.00-660.00] mm", "[0.00-660.00] mm",
                "[0.00-250.00] mm", "[0.00-360.00] Degree"]
var_x = tk.DoubleVar()
var_y = tk.DoubleVar()
var_z = tk.DoubleVar()
...
var_StartStop = [var_startstop1, var_startstop2,
                 var_startstop3, var_startstop4, var_startstop5]
...
def send_command_startstop():
    ISUS_UART.StartStop_Move(var_startstop1.get(), var_startstop2.get(
    ), var_startstop3.get(), var_startstop4.get(), var_startstop5.get())


def send_command_joint():
    ISUS_UART.Joint_Move(var_joint1.get(), var_joint2.get(),
                         var_joint3.get(), var_joint4.get())
...
def send_command_field_pick():
    ...
    ), var_filed3.get(), var_filed4.get(), var_chess.get())
...
if __name__ == '__main__':
    ...
    send_command2 = tk.Button(
        master=window, text="send_startstop", command=send_command_startstop)
    send_command2.place(relx=0.05, rely=0.85)

    send_command5 = tk.Button(
        master=window, text="send_home", command=send_command_home)
    send_command5.place(relx=0.05, rely=0.92)

    send_command1 = tk.Button(
        master=window, text="send_joint", command=send_command_joint)
    send_command1.place(relx=0.255, rely=0.85)

    send_command1 = tk.Button(
        master=window, text="send_xyz", command=send_command_XYZ)
    send_command1.place(relx=0.43, rely=0.85)

    send_command3 = tk.Button(
        master=window, text="send_grip_open", command=send_command_grip_open)
    send_command3.place(relx=0.6, rely=0.85)

    send_command4 = tk.Button(
        master=window, text="send_grip_close", command=send_command_grip_close)
    send_command4.place(relx=0.8, rely=0.85)

    send_command5 = tk.Button(
        master=window, text="Chess_Pick", command=send_command_field_pick)
    send_command5.place(relx=0.4, rely=0.92)

    send_command5 = tk.Button(
        master=window, text="Chess_Drop", command=send_command_field_drop)
